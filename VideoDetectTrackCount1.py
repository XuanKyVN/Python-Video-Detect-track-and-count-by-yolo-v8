...
from videodetecttractscreen import Ui_MainWindow
import imutils
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # ------------------------------------------------------
        self.disply_width = 1380
        self.display_height = 920
...
#----------------------------------------------
    def start(self):
        if self.video_path!="" and self.pretrainpath!="":
            ...
            data = {'classes': self.classes, 'Mode': self.Mode, 'v': self.video_path, 'pt': self.pretrainpath, 'flag':True}
            # print(data)
    ...
    def update_image(self, cv_img):
        ...
        cv_img = imutils.resize(cv_img, width = 1380)
        if len(self.MultilinePoint)>0:
            for data in self.MultilinePoint:
                for data1 in data:
                    cv2.line(cv_img,(data1[0],data1[1]),(data1[2],data1[3]),(0,255,0),2)
            #print(self.MultilinePoint[0])

        '''if len(self.rectpoint_original) > 0:
            for data in self.rectpoint_original:
                for data1 in data:
                    cv2.rectangle(cv_img,(data1[0],data1[1]),(data1[2],data1[3]),(255,0,0),2)'''
        if len(self.MutiPolypoint) > 0:
            for data in self.MutiPolypoint:
                data1= np.asarray(data, dtype=np.int32) #Convert to Numpy Array
                data1 = data1.reshape(-1, 1, 2)
                cv2.polylines(cv_img, [data1], True, (255, 0, 0), 2)
            ...
            while cap.isOpened():
                ...
                if success:
                    ...
                    for x in names.values():
                        ...
                        self.uic.Classes_Txt.append(str(index) + "  " + x)
                        index += 1
                    break
...
#----------------------------------------------DRAWN------------------
    ...
    def mousePressEvent(self, event):
        if event.buttons() & Qt.LeftButton and event.pos().x()<self.disply_width and event.pos().y()<self.display_height:
            #print('Point 1')
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() & Qt.LeftButton and event.pos().x()<self.disply_width and event.pos().y()<self.display_height and abs(self.begin.x()-self.destination.x()) >2 and not self.status:
            #print('Point 3')
            rect = QRect(self.begin, self.destination)
    ...
    def set_region(self):
        ...
        if len(self.PolygonPoint)>=1 and self.uic.ComboBox.currentIndex()==2:
            ...
            '''
            self.rectpoint_original.append(self.RecPoint.copy())
            ...
            temp =self.LinePoint.copy()
            self.MultilinePoint.append(temp)
